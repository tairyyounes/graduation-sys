<template>
  <section class="space-y-6">
    ...
    <div v-if="loading" class="text-center py-20 bg-white rounded-2xl border border-slate-200 shadow-sm">
      ...
      <p class="text-sm font-medium text-slate-500">Loading comparison details...</p>
    </div>
      ...
      <div class="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div v-else class="text-sm text-slate-500 bg-slate-50 p-4 rounded-xl border border-slate-100 max-w-sm">
            💡 No direct match detected in the AI index. This means your project is highly unique compared to this selected proposal!
          </div>
        </div>
      ...
      <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <article class="flex flex-col rounded-2xl border border-teal-200 bg-white shadow-sm overflow-hidden">
          <div class="border-b border-teal-100 bg-teal-50/50 p-5">
            ...
            <p class="mt-1 text-xs font-medium text-slate-500">Author: {{ currentProposal.author }}</p>
          </div>
        ...
        <article class="flex flex-col rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
          <div class="border-b border-slate-100 bg-slate-50/50 p-5">
            <div class="mb-3 inline-block rounded-full bg-slate-200 px-2.5 py-0.5 text-xs font-semibold text-slate-700">
              Compared Match ({{ comparedProposal.year }})
            </div>
            <h2 class="text-xl font-bold text-slate-900">{{ comparedProposal.title }}</h2>
            <p class="mt-1 text-xs font-medium text-slate-500">Author: {{ comparedProposal.author }} · {{ comparedProposal.department }}</p>
          </div>
...
async function fetchComparison() {
  ...
  try {
    ...
    if (res.ok) {
      ...
      const data = await res.json();
      error.value = data.message || 'Error loading comparison details.';
    }
  } catch (err) {
    console.error('Error fetching comparison:', err);
    error.value = 'Failed to fetch comparison details from the server.';
  } finally {
    loading.value = false;
  }
}
...
async function runSimilarityRecheck() {
  ...
  checking.value = true;
  toast.info('Starting similarity check...');
  try {
    ...
    if (res.ok) {
      toast.success('Similarity analysis updated.');
      await fetchComparison(); // reload values
    } else {
      toast.error('Failed to trigger recheck.');
    }
  } catch (err) {
