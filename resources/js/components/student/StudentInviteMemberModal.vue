    ...
    <div class="flex min-h-screen items-center justify-center p-4 text-center sm:p-0">
      ...
      <div class="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-xl transition-all sm:my-8 w-full max-w-md flex flex-col">
        ...
        <div class="border-b border-slate-100 px-6 py-5 bg-slate-50/50 flex justify-between items-center">
          <h3 class="text-lg font-bold text-slate-900">{{ $t('student.invite.title') }}</h3>
        ...
        <!-- Body -->
        <div class="px-6 py-6">
          <p class="text-sm text-slate-500 mb-4">{{ $t('student.invite.description') }}</p>
          <div>
            <label class="block text-sm font-medium text-slate-700 mb-1">{{ $t('student.team.reg_number') }}</label>
            <input
              :value="regNumber"
              ...
              :placeholder="$t('student.invite.reg_placeholder')"
            >
